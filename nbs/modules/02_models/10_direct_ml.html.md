...
::: {#e9b097ed .cell 0='e' 1='x' 2='p' 3='o' 4='r' 5='t'}
``` {.python .cell-code}
...
ml_linear = ml_direct_forecaster(model=XGBRegressor(n_estimators=100, verbose=-1),
              target_col='admissions', H = [5, 10],  lags = 14,
              cat_variables=cat_variables, categorical_encoder=ohe,
              future_exog_cols=cat_variables)
ml_linear.fit(train)
# ml_linear.data_prep(train)
forecasts = ml_linear.forecast(H=5, exog=test[cat_variables])
...
::: {#91e0461a .cell 0='h' 1='i' 2='d' 3='e'}
``` {.python .cell-code}
from peshbeen.metrics import WMAPE, MAE, RMSE
mtrcs = [WMAPE, MAE, RMSE]
cv_results = ml_linear.cross_validate(df=train, cv_split=10, metrics=mtrcs, step_size=3)
cv_results.head()
```
