...
::: {#3043b75e .cell 0='e' 1='x' 2='p' 3='o' 4='r' 5='t'}
``` {.python .cell-code}
...
class var:
    ...
    def forecast(
        ...
        for t in range(H):
            ...
            inp = cons + exo_inp + inp_lag + transform_lag
...
var_model = var(target_cols=['admissions', "calls"], lags={'admissions': 2, "calls": 1}, box_cox={'admissions': False, "calls": False},
                cat_variables=cat_variables, categorical_encoder={'admissions': te},
...
![](04_var_files/figure-html/cell-4-output-1.png){}
:::
:::


::: {#7dcd35fe .cell 0='h' 1='i' 2='d' 3='e'}
``` {.python .cell-code}
from peshbeen.metrics import WMAPE, MAE, RMSE
mtrcs = [WMAPE, MAE, RMSE]
cvdf = var_model.cross_validate(df=admission_calls, target_col='calls', cv_split=3, test_size=30, metrics=mtrcs, h_split_point=15)
var_model.cv_summary
```
